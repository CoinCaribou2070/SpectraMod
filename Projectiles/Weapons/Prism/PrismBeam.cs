...
namespace SpectraMod.Projectiles.Weapons.Prism
{
    public abstract class PrismBeam : ModProjectile
    {
		...
		/// </summary>
		protected abstract float BeamHitboxCollisionWidth();
		...
		/// </summary>
		protected abstract float BeamLightBrightness();
		...
		protected abstract float BeamColorHue();
		protected abstract float BeamHueVariance();
		protected abstract float BeamColorSaturation();
		protected abstract float BeamColorLightness();

		protected abstract int ParentType();

		protected abstract PrismHoldout Parent();

		protected abstract int HitCD();
		...
		public override void AI()
		{
			...
			//if (projectile.type != ParentType() || !hostPrism.active || hostPrism.type != ParentType())
			//if (Main.player[projectile.owner].ownedProjectileCounts[ParentType()] < 1)
			if (!Main.player[projectile.owner].channel || !Parent().ShouldConsumeMana() || !Main.player[projectile.owner].CheckMana(Main.player[projectile.owner].HeldItem.mana, false))
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / Parent().ChargeTime(), 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > Parent().DamageStart();

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - Parent().NumBeams() / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * Parent().NumBeams()) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / Parent().NumBeams()) % BeamHueVariance() + BeamColorHue();
